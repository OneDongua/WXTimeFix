...
public class FileUtils {
    ...
    public static String getPathFromUri(Context context, Uri uri) {
        ...
        // 优先处理文档树URI (tree document)，不依赖 isDocumentUri 判断
        if (isTreeDocumentUri(uri)) {
            Log.d(TAG, "Is tree document URI");
            try {
                String docId = DocumentsContract.getTreeDocumentId(uri);
                Log.d(TAG, "Tree document ID: " + docId);
                
                // 移除 "primary:" 前缀
                if (docId.startsWith("primary:")) {
                    docId = docId.substring(8);
                }
                String result = Environment.getExternalStorageDirectory() + "/" + docId;
                Log.d(TAG, "Result path: " + result);
                return result;
            } catch (Exception e) {
                Log.e(TAG, "Error processing tree document URI", e);
                return null;
            }
        }
        ...
        if (DocumentsContract.isDocumentUri(context, uri)) {
            Log.d(TAG, "Is document URI");
            
            // ExternalStorageProvider（非tree格式）
